...
import type { ModelInfo, ProviderInfo } from '@recoder-neo/schema'
                ...
                {:else}
                    <div class="space-y-3">
                        <div><label class="text-sm font-medium" for="docker_img">Worker Image</label>
                            <input id="docker_img" type="text" class="mt-1 flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="recoder-dev002.develop.10.199.64.20.nip.io/recoder-worker:latest" value={values["worker_image"] || ""} oninput={(e) => values = { ...values, worker_image: e.currentTarget.value }} /></div>
            ...
            {:else if activeId === 'advanced'}
                <div class="grid grid-cols-1 gap-3">
                    <div>
                        ...
                        <input id="repos_root" type="text" class="mt-1 flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="/home/user/.recoder/repos" value={values["repos_root"] || ""} oninput={(e) => values = { ...values, repos_root: e.currentTarget.value }} />
                    </div>
                    <div>
